function Xdot = GravAcc(t,inert_sv, we, degree, order, mu, Re, Clm, Slm)
...
% X_ITRF = DCM*inert_sv(1:3,1);

rot_angle1 = we*t; %nondim w * nondim time
rot_z = [cos(rot_angle1), sin(rot_angle1), 0; -sin(rot_angle1), cos(rot_angle1), 0; 0, 0, 1];

Rot_Matr = rot_z; %rotation matrix

if iscolumn(inert_sv)
    rot_r = Rot_Matr*inert_sv(1:3); % state vector in moon equator plane
else
    rot_r = Rot_Matr*inert_sv(1:3)'; % state vector in moon equator plane
end
...
% Transform to GCRF
Xdot(1:3,1) = inert_sv(4:6);
%Xdot(4:6,1) = DCM'*a_ITRF;
Xdot(4:6,1) = Rot_Matr'*a_ITRF;
